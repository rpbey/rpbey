  ...
  public override async chatInputRun(
    ...
    try {
      ...
      if (format === 'text') {
        ...
        textOutput += 'RANG | BLADER               | PTS   | WIN% | W   | L   | TOURNOIS\n';
        textOutput += '-----+----------------------+-------+------+-----+-----+---------\n';

        profiles.forEach((p, index) => {
          const rank = (index + 1).toString().padEnd(4);
          const name = (p.bladerName || p.user.name || 'Inconnu').substring(0, 20).padEnd(20);
          const points = p.rankingPoints.toString().padEnd(5);
          const totalMatches = p.wins + p.losses;
          const winRate = (totalMatches > 0 ? ((p.wins / totalMatches) * 100).toFixed(1) : '0.0').padStart(4);
        ...
        const attachment = new AttachmentBuilder(buffer, { name: 'classement-rpb.txt' });
      ...
      if (format === 'csv') {
        // Generate CSV
        const header = 'Rang,Blader,Pseudo Discord,Points,Victoires,Defaites,Matches,Winrate,Tournois,DateInscription\n';
        const rows = profiles.map((p, index) => {
            const totalMatches = p.wins + p.losses;
            const winRate = totalMatches > 0 ? ((p.wins / totalMatches) * 100).toFixed(2) : '0';
        ...
        const attachment = new AttachmentBuilder(buffer, { name: 'classement-rpb.csv' });
