...
export class RankingCommand extends Subcommand {
    ...
    registry.registerChatInputCommand((builder) =>
      builder
        ...
        .addSubcommand((command) =>
          command
            .setName('reset')
            .setDescription('RAZ complet des points de classement de tous les membres'),
        )
        .addSubcommand((command) =>
          command
            .setName('setup')
            ...
            .addIntegerOption((option) =>
              option.setName('match_win').setDescription('Points par victoire de match (ex: 200)').setRequired(true),
            )
            .addIntegerOption((option) =>
              option.setName('participation').setDescription('Points de participation (ex: 500)').setRequired(true),
            )
            .addIntegerOption((option) =>
              option.setName('top8').setDescription('Points pour Top 8 (ex: 500)').setRequired(true),
            )
            .addIntegerOption((option) =>
              option.setName('third').setDescription('Points pour 3ème place (ex: 1000)').setRequired(true),
            )
            .addIntegerOption((option) =>
              option.setName('second').setDescription('Points pour 2ème place (ex: 1500)').setRequired(true),
            )
            .addIntegerOption((option) =>
              option.setName('first').setDescription('Points pour 1ère place (ex: 2000)').setRequired(true),
            ),
        ),
    );
  }
  ...
  public async chatInputReset(interaction: Subcommand.ChatInputCommandInteraction) {
    ...
    try {
      ...
      return interaction.editReply(`✅ Classement réinitialisé avec succès.\nPoints remis à 0 pour ${result.count} profils.`);
    } catch (error) {
      this.container.logger.error(error);
      return interaction.editReply('❌ Une erreur est survenue lors de la réinitialisation.');
    }
  }
