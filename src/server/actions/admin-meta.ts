...
export async function getMetaStats() {
  ...
  return {
    blades: bladeUsage.map(u => ({ ...partMap.get(u.bladeId!), count: u._count.bladeId })),
    ratchets: ratchetUsage.map(u => ({ ...partMap.get(u.ratchetId!), count: u._count.ratchetId })),
    bits: bitUsage.map(u => ({ ...partMap.get(u.bitId!), count: u._count.bitId })),
    assists: assistUsage.map(u => ({ ...partMap.get(u.assistBladeId!), count: u._count.assistBladeId }))
  };
}
