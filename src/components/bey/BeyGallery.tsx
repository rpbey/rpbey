...
import type { BeyManifest, BeyFile } from '@/types/bey';
...
export function BeyGallery({ manifest }: BeyGalleryProps) {
  const [selectedCategory, setSelectedCategory] = useState(manifest.categories[0]);
  ...
  const defaultTexture = useMemo(() => {
    return manifest.textures.find(t => t.name === 'DRANSWORD') || manifest.textures[0];
  }, [manifest]);
  ...
  const filteredModels = useMemo(() => {
    return manifest.models.filter(model => {
      ...
      const matchesSearch = model.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
                            (model.subcategory && model.subcategory.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  return (
    <Grid container spacing={3} sx={{ height: 'calc(100vh - 100px)' }}>
      ...
      <Grid size={{ xs: 12, md: 4, lg: 3 }} sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
        ...
        <Paper sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRadius: 4 }}>
          <Tabs
            ...
            <Stack spacing={1}>
              {filteredModels.map((model) => (
                <Card 
                  ...
                  sx={{ 
                    borderRadius: 2,
                    borderColor: selectedModel?.path === model.path ? 'primary.main' : 'divider',
                    bgcolor: selectedModel?.path === model.path ? 'action.selected' : 'background.paper'
                  }}
                >
                  <CardActionArea onClick={() => setSelectedModel(model)} sx={{ p: 1.5 }}>
      ...
      <Grid size={{ xs: 12, md: 8, lg: 9 }} sx={{ height: '100%' }}>
        <Paper sx={{ height: '100%', p: 2, borderRadius: 4, display: 'flex', flexDirection: 'column', gap: 2 }}>
          {selectedModel ? (
            <>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <Chip label={selectedModel.category} color="primary" size="small" />
              </Box>
                  ...
                  {manifest.textures.map((tex) => (
                    <Box
                      ...
                      sx={{
                        ...
                        overflow: 'hidden',
                        border: '2px solid',
                        borderColor: currentTexture?.path === tex.path ? 'primary.main' : 'transparent',
                        cursor: 'pointer',
                        '&:hover': { opacity: 0.8 }
                      }}
                    >
            ...
            <Box sx={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 2, opacity: 0.5 }}>
              <CategoryIcon sx={{ fontSize: 64 }} />
              <Typography variant="h6">Sélectionnez une pièce pour l'afficher en 3D</Typography>
            </Box>
          )}
        </Paper>
      </Grid>
