...
export function SatrCharts({ bladers }: SatrChartsProps) {
  ...
  const pieData = Object.entries(winrateRanges).map(([label, value], id) => ({
    ...
    value,
    label,
    color: id === 0 ? '#fbbf24' : id === 1 ? '#4caf50' : id === 2 ? '#ff9800' : '#f44336'
  }));

  return (
    <Box sx={{ mb: 6 }} component={motion.div} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
      <Grid container spacing={3}>
        ...
        <Grid size={{ xs: 12, md: 7 }}>
          <Paper sx={{ 
            ...
            <Typography variant="subtitle2" sx={{ fontWeight: 900, mb: 2, color: '#fbbf24', textTransform: 'uppercase', letterSpacing: 1 }}>
            ...
            <Box sx={{ width: '100%', height: 280 }}>
                <BarChart
                    ...
                    series={[{ dataKey: 'wins', color: '#fbbf24', label: 'Victoires' }]}
                    slotProps={{
                        legend: { hidden: true }
                    } as any}
                    sx={{
        ...
        <Grid size={{ xs: 12, md: 5 }}>
          <Paper sx={{ 
            ...
            <Typography variant="subtitle2" sx={{ fontWeight: 900, mb: 2, color: '#fff', textTransform: 'uppercase', letterSpacing: 1 }}>
