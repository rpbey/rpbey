...
export async function POST(req: Request) {
  ...
  // Security Check
  const authorized = (process.env.GEMINI_API_KEY && token === process.env.GEMINI_API_KEY) || 
  ...
  try {
    ...
    switch (action) {
      case 'status': {
        const [users, tournaments, profiles, activeTournaments] = await Promise.all([
          prisma.user.count(),
      ...
      case 'get_schema': {
        ...
        return NextResponse.json({ 
          models: ['User', 'Tournament', 'Profile', 'Beyblade', 'Part', 'Deck', 'TournamentMatch'],
      ...
      case 'sql_query': {
        ...
        if (!upperQuery.startsWith('SELECT') && !payload.forceUnsafe) {
           return NextResponse.json({ error: 'Only SELECT queries are allowed by default. Use forceUnsafe: true to override.' }, { status: 403 });
        }

        try {
          const result = await prisma.$queryRawUnsafe(query);
          // Handle BigInt serialization
          const sanitized = JSON.parse(JSON.stringify(result, (_, v) =>
      ...
      case 'read_log': {
        ...
        if (logType === 'bot') {
            try {
                const content = await readFile(join(process.cwd(), 'bot/bot.log'), 'utf-8');
                // Return last 2000 chars
                return NextResponse.json({ content: content.slice(-2000) });
            } catch {
                return NextResponse.json({ error: 'Log file not found or empty' }, { status: 404 });
            }
        }
      ...
      case 'manage_user': {
        ...
        if (!userId || !userAction) return NextResponse.json({ error: 'Missing userId or action' }, { status: 400 });

        if (userAction === 'ban') {
           const user = await prisma.user.update({
             where: { id: userId },
             data: { banned: true, banReason: reason || 'Banned by Gemini Agent' }
           });
           return NextResponse.json({ success: true, user: { id: user.id, banned: user.banned } });
        }
