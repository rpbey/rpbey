...
import { Link as LinkIcon, SwapHoriz, Search, CheckCircle, PersonOff } from '@mui/icons-material';
import { useEffect, useState, useTransition } from 'react';
import { toast } from 'sonner';
import { getUnlinkedParticipants, getAllRealUsers, mergeUserAccounts } from '@/server/actions/admin-link';
  ...
  return (
    <Box>
      <Box sx={{ mb: 4, display: 'flex', alignItems: 'center', gap: 2 }}>
        ...
        <Box>
          <Typography variant="h4" fontWeight="900">LIAISON DE COMPTES</Typography>
      ...
      {!activeTournament ? (
        ...
        <Grid container spacing={3}>
          {activeTournament.participants.map((p: any) => {
            const isPlaceholder = p.user.email.includes('placeholder.rpb') || p.user.email.includes('discord.rpb');
            const hasDiscordId = !!p.user.discordId;

            return (
              <Grid size={{ xs: 12 }} key={p.id}>
                <Card sx={{ borderRadius: 3, border: '1px solid', borderColor: isPlaceholder ? 'warning.main' : 'divider' }}>
                  <CardContent sx={{ p: 2 }}>
                      ...
                      <Grid size={{ xs: 12, md: 4 }}>
                        <Stack direction="row" spacing={2} alignItems="center">
                          ...
                          <Box sx={{ minWidth: 0 }}>
                            ...
                            {isPlaceholder ? (
                              <Chip label="TEMP" size="small" color="warning" sx={{ height: 16, fontSize: '0.6rem', fontWeight: 900, mt: 0.5 }} />
                            ) : hasDiscordId ? (
                              <Chip label="LIÉ" size="small" color="success" icon={<CheckCircle sx={{ fontSize: '10px !important' }} />} sx={{ height: 16, fontSize: '0.6rem', fontWeight: 900, mt: 0.5 }} />
                            ) : null}
                          </Box>
                        </Stack>
                      </Grid>
                            ...
                            renderOption={(props, option) => (
                              <Box component="li" {...props} sx={{ gap: 2 }}>
                                ...
                                <Box>
                                  ...
                                  <Typography variant="caption" color="text.secondary">@{option.discordTag}</Typography>
                                </Box>
                              </Box>
                            )}
                            onChange={(_, newValue) => {
                              ...
                              <TextField 
                                ...
                                slotProps={{
                                  input: {
                                    ...
                                    startAdornment: (
                                      <>
                                        <Search fontSize="small" sx={{ mr: 1, color: 'text.disabled' }} />
