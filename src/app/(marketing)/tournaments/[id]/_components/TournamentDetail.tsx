    ...
    <Box sx={{ width: '100%', py: { xs: 2, md: 4 }, px: { xs: 2, md: 4, lg: 6 } }}>
      ...
      <Grid container spacing={{ xs: 3, md: 5 }}>
          ...
          <Stack spacing={3} sx={{ position: { lg: 'sticky' }, top: { lg: 100 } }}>
            ...
            <Paper 
              ...
              sx={{ 
                ...
                background: isBTS ? 'linear-gradient(135deg, #1a0a0a 0%, #0a0a0a 100%)' : 'background.paper' 
              }}
            >
              <Stack spacing={3}>
                <Box>
                  <Typography variant="caption" color="text.secondary" fontWeight={900} sx={{ display: 'block', mb: 0.5, letterSpacing: 1 }}>DATE & HEURE</Typography>
                ...
                <Box>
                  <Typography variant="caption" color="text.secondary" fontWeight={900} sx={{ display: 'block', mb: 0.5, letterSpacing: 1 }}>LIEU</Typography>
                  <Typography variant="body1" fontWeight={800}>{tournament.location}</Typography>
                </Box>

                <Box>
                  <Typography variant="caption" color="text.secondary" fontWeight={900} sx={{ display: 'block', mb: 0.5, letterSpacing: 1 }}>FORMAT</Typography>
                  <Typography variant="body1" fontWeight={800}>{tournament.format}</Typography>
                  <Typography variant="caption" color="text.disabled" fontWeight={700}>Capacité: {tournament.maxPlayers} joueurs</Typography>
                </Box>
                  ...
                  {session && (
                    <Box sx={{ p: 2, borderRadius: 2, bgcolor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)' }}>
                      {profileData?.challongeUsername ? (
                        ...
                        <Button fullWidth size="small" variant="outlined" component={Link} href={`/api/auth/challonge?returnTo=/tournaments/${tournament.id}`} sx={{ color: '#fbbf24', borderColor: '#fbbf24', fontWeight: 900, fontSize: '0.7rem' }}>LIER MON COMPTE CHALLONGE</Button>
                      )}
                    </Box>
                  )}
                  {tournament.challongeUrl && (
                    <Button variant="contained" fullWidth href={tournament.challongeUrl} target="_blank" sx={{ py: 2, fontWeight: 900, bgcolor: '#dc2626', fontSize: '1.1rem' }}>S&apos;INSCRIRE MAINTENANT</Button>
                  )}
                  <Button variant="outlined" fullWidth href="https://discord.gg/rpb" target="_blank" sx={{ color: isBTS ? 'white' : 'text.primary', borderColor: isBTS ? 'rgba(255,255,255,0.2)' : 'divider' }}>REJOINDRE LE DISCORD</Button>
            ...
            {/* Small Map */}
            <Paper elevation={0} sx={{ height: 250, borderRadius: 6, border: '1px solid', borderColor: 'divider', overflow: 'hidden' }}>
        ...
        <Grid size={{ xs: 12, lg: 8, xl: 9 }} sx={{ order: { xs: 2, lg: 1 } }}>
                    ...
                    <Paper elevation={0} sx={{ p: 3, borderRadius: 4, border: '1px solid', borderColor: 'divider', bgcolor: 'background.paper' }}>
                      ...
                      {station.currentMatch ? (
                        <Stack direction="row" justifyContent="space-between" alignItems="center">
                          ...
                          <Typography variant="caption" fontWeight="900" sx={{ px: 1.5, py: 0.5, bgcolor: 'error.main', color: 'white', borderRadius: 1 }}>{station.currentMatch.scores || 'VS'}</Typography>
          ...
          {/* About Section */}
          <Paper elevation={0} sx={{ p: { xs: 3, md: 6 }, mb: 4, borderRadius: 6, border: '1px solid', borderColor: isBTS ? alpha('#fbbf24', 0.2) : 'divider', background: isBTS ? 'linear-gradient(180deg, rgba(255,255,255,0.01) 0%, transparent 100%)' : 'background.paper' }}>
            <Typography variant="h5" fontWeight="900" sx={{ mb: 4, display: 'flex', alignItems: 'center', gap: 2, color: isBTS ? '#fbbf24' : 'primary.main', letterSpacing: 1 }}><Info /> À PROPOS DU TOURNOI</Typography>
            <ReactMarkdown
              ...
              components={{
                p: ({ children }) => {
                  ...
                  if (content.includes('🥇') || content.includes('🥈') || content.includes('🥉')) {
                    return <Box sx={{ my: 2, p: 2.5, bgcolor: 'rgba(251, 191, 36, 0.08)', borderRadius: 3, borderLeft: '6px solid #fbbf24' }}><Typography variant="h6" fontWeight="900" sx={{ color: '#fff', m: 0 }}>{children}</Typography></Box>;
                  }
                  if (content.includes('⚠️')) {
                    return <Box sx={{ my: 3, p: 3, bgcolor: 'rgba(239, 68, 68, 0.15)', borderRadius: 4, border: '2px solid rgba(239, 68, 68, 0.3)' }}><Typography variant="body1" fontWeight="800" sx={{ color: '#ef4444', m: 0, lineHeight: 1.6 }}>{children}</Typography></Box>;
                  }
                  return <Typography variant="body1" sx={{ mb: 3, lineHeight: 2, fontSize: '1.15rem', color: isBTS ? 'grey.300' : 'text.primary', fontWeight: 500 }}>{children}</Typography>;
                },
                strong: ({ children }) => <Box component="span" sx={{ fontWeight: 900, color: isBTS ? '#fff' : 'text.primary' }}>{children}</Box>,
                a: ({ href, children }) => <Box component="a" href={href} target="_blank" sx={{ color: 'error.main', textDecoration: 'none', fontWeight: 700, '&:hover': { textDecoration: 'underline' } }}>{children}</Box>
              }}
            >
          ...
          <Paper id="tournament-view" elevation={0} sx={{ borderRadius: 8, border: '1px solid', borderColor: 'divider', overflow: 'hidden', boxShadow: '0 20px 50px rgba(0,0,0,0.1)' }}>
            <Tabs 
              value={activeTab} 
              ...
              sx={{ 
                ...
                borderColor: 'divider',
                '& .MuiTab-root': { fontWeight: 900, minHeight: 70, fontSize: '1rem' } 
              }}
            >
            ...
            <Box sx={{ p: { xs: 2, md: 6 } }}>
              {activeTab === 0 && tournament.challongeUrl && (
                <ChallongeBracket challongeUrl={tournament.challongeUrl} title={tournament.name} />
              )}
...
function StandingsPanel({ standings }: { standings: Standing[] }) {
  ...
  return (
    <Stack spacing={1.5}>
      {standings.length === 0 && <Typography variant="body2" color="text.secondary" textAlign="center" sx={{ py: 4 }}>Classement non disponible pour le moment.</Typography>}
      {standings.map((s) => {
        const isTop3 = s.rank <= 3;
        const color = s.rank === 1 ? '#fbbf24' : s.rank === 2 ? '#94a3b8' : s.rank === 3 ? '#d97706' : 'transparent';
        return (
          <Box key={s.rank} sx={{ display: 'flex', alignItems: 'center', gap: 3, p: 2.5, borderRadius: 4, border: '1px solid', borderColor: isTop3 ? alpha(color, 0.3) : 'divider', bgcolor: isTop3 ? alpha(color, 0.03) : 'transparent' }}>
            <Box sx={{ width: 40, height: 40, borderRadius: 2, bgcolor: isTop3 ? color : 'action.selected', color: s.rank === 1 ? 'black' : 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, fontSize: '1.1rem' }}>{s.rank}</Box>
            <Typography variant="h6" fontWeight={900} sx={{ flex: 1 }}>{s.name}</Typography>
            <Typography variant="subtitle1" fontWeight={900} color="success.main">{s.stats?.wins ?? s.wins}W <Box component="span" sx={{ color: 'text.disabled', mx: 0.5 }}>-</Box> <Box component="span" sx={{ color: 'error.main' }}>{s.stats?.losses ?? s.losses}L</Box></Typography>
            {s.challongeProfileUrl && <Button size="small" variant="outlined" href={s.challongeProfileUrl} target="_blank" sx={{ borderRadius: 2, fontWeight: 800 }}>Profil</Button>}
          </Box>
        );
      })}
    </Stack>
...
function ActivityLogPanel({ log }: { log: LogEntry[] }) {
  ...
  return (
    <Stack spacing={2}>
      {log.length === 0 && <Typography variant="body2" color="text.secondary" textAlign="center" sx={{ py: 4 }}>Aucune activité récente.</Typography>}
      {log.slice(0, 30).map((entry, i) => (
        <Box key={i} sx={{ p: 2.5, borderRadius: 4, bgcolor: alpha(theme.palette.action.hover, 0.4), borderLeft: '4px solid', borderColor: entry.type?.includes('match') ? 'primary.main' : 'divider' }}>
          <Stack direction="row" justifyContent="space-between" sx={{ mb: 1 }}>
            <Typography variant="caption" fontWeight={900} sx={{ textTransform: 'uppercase', color: 'primary.main', letterSpacing: 1 }}>{entry.type}</Typography>
            <Typography variant="caption" color="text.secondary" fontWeight={700}>{new Date(entry.timestamp).toLocaleTimeString('fr-FR')}</Typography>
          </Stack>
