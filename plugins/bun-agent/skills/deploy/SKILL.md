---
name: bun-deploy
description: "Deploy application to production. TRIGGER when: user says 'deploy', 'ship', 'push to prod', 'release', 'mise en prod', or asks to restart a service. Handles build verification, Docker, systemd, and deployment validation."
...
2. Pass it the deployment target based on user request:
   - "dashboard" or "web" → Docker dashboard deployment
   - "bot" → systemd bot deployment
