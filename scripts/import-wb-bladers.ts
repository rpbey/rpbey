...
async function run() {
  console.log('🚀 Synchronisation et Liaison des profils Wild Breakers...');

  try {
    const jsonPath = join(process.cwd(), 'data', 'wb_blader_profiles.json');
    ...
    try {
      ...
      console.log(`🧹 Nettoyage de la table wb_bladers...`);
      await client.query('DELETE FROM wb_bladers');
      ...
      for (const p of profiles) {
        ...
        await client.query(
          'INSERT INTO wb_bladers (id, name, "totalWins", "totalLosses", "tournamentWins", "tournamentsCount", history, "linkedUserId", "updatedAt") VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW())',
          [
            `wb_${Math.random().toString(36).substr(2, 9)}`,
            p.name,
            p.totalWins,
            p.totalLosses,
            p.tournamentWins || 0,
